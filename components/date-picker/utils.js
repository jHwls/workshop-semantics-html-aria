...
export const createPrevMonthDays = (
	year,
	month,
	activeMonthDays,
	...
	const visibleDaysFromPrevMonth = firstDayOfMonthWeekday
		? firstDayOfMonthWeekday
		: 6;

	const prevMonthLastMondayDayOfMonth = dayjs(activeMonthDays[0].date)
	...
	return [...Array(visibleDaysFromPrevMonth)].map((day, index) => {
		let date = dayjs(
			`${prevMonth.year()}-${prevMonth.month() + 1}-${
				prevMonthLastMondayDayOfMonth + index
			}`
		).format("YYYY-MM-DD");
		return {
			date: date,
...
export const createNextMonthDays = (
	year,
	month,
	activeMonthDays,
	initUnavailableDates
	...
	const lastDayOfMonthWeekday = getWeekday(
		`${year}-${month}-${activeMonthDays.length}`
	);
	const visibleDaysFromNextMonth = lastDayOfMonthWeekday
		? 6 - lastDayOfMonthWeekday
		: lastDayOfMonthWeekday;

	return [...Array(visibleDaysFromNextMonth)].map((day, index) => {
		let date = dayjs(`${year}-${Number(month + 1)}-${index + 1}`).format(
			"YYYY-MM-DD"
		);
		return {
			date: date,
